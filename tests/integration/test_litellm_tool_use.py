...
def test_litellm_models_tool_compatibility(model_id, toolkit):
    ...
    try:
        ...
        assert len(response_text) > 0, f"Model {model_id} did not return a valid response"
        ...
        if not tools_mentioned:
            print(f"Note: Model {model_id} did not explicitly mention tool names in response")
